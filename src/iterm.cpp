...
#include "filters/averageFilter.hpp"
...
void ITerm::setupCommands() {
  ...
  m_commandMap["filter"] = &ITerm::filter;
  m_commandMap["f"] = &ITerm::filter;
  m_commandMap["background"] = &ITerm::background;
  m_commandMap["bg"] = &ITerm::background;
  m_commandMap["fill"] = &ITerm::fillShape;
  m_commandMap["rotate"] = &ITerm::rotate;
  m_commandMap["rot"] = &ITerm::rotate;
  m_commandMap["translate"] = &ITerm::translate;
  m_commandMap["tr"] = &ITerm::translate;
  m_commandMap["scale"] = &ITerm::scale;
  m_commandMap["sc"] = &ITerm::scale;
...
void ITerm::createEllipse(std::string name) {
  std::cout << "Format: <radiusX> <radiusY> <center.x center.y>\n\n  └ ";

  std::string line;
  std::getline(std::cin, line);
  std::istringstream iss{line};

  auto radiusX = readNext<double>(iss, "  [LOG] Erro ao ler raio em X.");

  if (!radiusX.has_value())
    return;

  auto radiusY = readNext<double>(iss, "  [LOG] Erro ao ler raio em Y.");

  if (!radiusY.has_value())
    return;
  ...
  auto ellipse = std::make_unique<Ellipse>(radiusX.value(), radiusY.value(),
                                           center.value());

  auto new_canvas = std::make_unique<Canvas>(*m_canvas.top());
  ellipse->drawObject(*new_canvas, {0, 0, 0});

  m_canvas.push(std::move(new_canvas));

  storeShape(std::move(name), std::move(ellipse));
}
...
void ITerm::scanLineFill(std::istringstream &iss) {
...
void ITerm::filter(std::istringstream &iss) {
  auto type = readNext<std::string>(iss, "Faltando parâmetro (tipo)! "
                                         "Use: filter <average|median> "
                                         "[kernel]");
  if (!type.has_value())
    return;

  int kernel = 3; /// Default: vizinhança 3x3.
  if (!(iss >> kernel))
    kernel = 3;

  if (kernel < 1 || kernel % 2 == 0) {
    std::cout << "Kernel deve ser ímpar e >= 1.\n";
    return;
  }

  std::unique_ptr<Filter> f;
  if (type.value() == "average" || type.value() == "avg")
    f = std::make_unique<AverageFilter>(kernel);
  else if (type.value() == "median" || type.value() == "med")
    f = std::make_unique<MedianFilter>(kernel);
  else {
    std::cout << "Filtro '" << type.value()
              << "' inválido. Use: average ou median.\n";
    return;
  }

  auto new_canvas = std::make_unique<Canvas>(*m_canvas.top());
  f->applyFilter(*new_canvas);
  m_canvas.push(std::move(new_canvas));
...
void ITerm::applyTransform(SceneItem &item, const Transform &t) {
  item.shape->transform(t);
  renderShapes();
  std::cout << "Done.\n";
}
...
void ITerm::rotate(std::istringstream &iss) {
  auto name = readNext<std::string>(
      iss, "Faltando nome! Use: rotate <nome> <graus> [pivo.x pivo.y]");
  if (!name.has_value())
    return;
  ...
  auto degrees = readNext<double>(iss, "  [LOG] Erro ao ler ângulo (graus).");
  if (!degrees.has_value())
    return;

  /// Pivô default: o eixo do próprio objeto. Pivô explícito => órbitas.
  Point2 pivot = item->shape->getAxis();
  if (int px; iss >> px) {
    auto py = readNext<int>(iss, "  [LOG] Pivô incompleto (faltou pivo.y).");
    if (!py.has_value())
      return;
    pivot = Point2(px, py.value());
  }

  Transform t;
  t.rotate(degrees.value(), pivot);
  applyTransform(*item, t);
}

void ITerm::translate(std::istringstream &iss) {
  auto name = readNext<std::string>(
      iss, "Faltando nome! Use: translate <nome> <dx> <dy>");
  if (!name.has_value())
    return;
  ...
  auto delta = getPoint(iss);
  if (!delta.has_value())
    return;

  Transform t;
  t.translate(delta.value());
  applyTransform(*item, t);
}

void ITerm::scale(std::istringstream &iss) {
  auto name = readNext<std::string>(
      iss, "Faltando nome! Use: scale <nome> <sx> <sy> [pivo.x pivo.y]");
  if (!name.has_value())
    return;
  ...
  auto sx = readNext<double>(iss, "  [LOG] Erro ao ler fator sx.");
  if (!sx.has_value())
    return;

  auto sy = readNext<double>(iss, "  [LOG] Erro ao ler fator sy.");
  if (!sy.has_value())
    return;
  ...
  Transform t;
  t.scale(sx.value(), sy.value(), pivot);
  applyTransform(*item, t);
}
