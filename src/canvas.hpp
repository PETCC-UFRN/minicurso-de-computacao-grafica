 ...
 * @details Esta classe guarda as informações da imagem e permite a exportação
 * da mesma.
 * @date 25/05/2026()
 */
...
/**
 * @class Canvas
 * @brief Classe responsável por guardar as informações principais sobre a
 * imagem e exportá-la para os tipos PNG e PPM3
 */
class Canvas {
private:
...
public:
   ...
   *
   */
  Canvas(const int &w, const int &h, const std::string &fn, bool gc = false,
         ImgType imgType = ImgType::PPM3)
      : filename(fn), img_width(w), img_height(h), type(imgType),
        gamma_correction(gc) {
    buffer.resize(w * h);
  }
  ...
  void add(const Pixel &pixel, const RGBColor &color) {
    if (pixel.x() < 0 || pixel.x() >= img_width || pixel.y() < 0 ||
        pixel.y() >= img_height)
      return;
   ...
   * @details Utilizando o parâmetro imgType passado pelo construtor, ela define
   * qual o formato da imagem.
   */
  bool export_img() const {

    if (type == ImgType::PNG) {
      return ExportImg::exportPng(filename, buffer, img_width, img_height,
                                  gamma_correction);
    } else {
      return ExportImg::exportPPM3(filename, buffer, img_width, img_height,
                                   gamma_correction);
    }
  };
  ...
  const std::vector<RGBColor> &data() const {
    return buffer;
  } //> Getter da image;
