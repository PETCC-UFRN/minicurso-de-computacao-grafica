...
void Line::drawObject(Canvas &canvas, RGBColor color, DrawMethod method) {
  ...
  case DrawMethod::Bresenhan: {
    ...
    if (dx >= dy) {
      // Linha mais horizontal -> desloca em Y
      ...
      // Linha mais vertical -> desloca em X
      for (int t = 1; t <= half; t++) {
        drawBresenhan(Point2(start.x() - t, start.y()),
                      Point2(end.x() - t, end.y()), canvas, color);
  ...
  case DrawMethod::BresenhanMidpoint: {
    ...
    if (dx >= dy) {
      ...
      // Linha mais vertical -> desloca em X
      for (int t = 1; t <= half; t++) {
        drawMidpoint(Point2(start.x() - t, start.y()),
                      Point2(end.x() - t, end.y()), canvas, color);
...
void Line::transform(const Transform &transformation) {
  this->start = transformation * start;
  this->end = transformation * end;
}

Point2 Line::getAxis() const {
  Point2 average = (start + end) / 2;
