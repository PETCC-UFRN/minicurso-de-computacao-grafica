...
{
  private:
    array<RGBColor, 4> m_corners{RGBColor(), RGBColor(), RGBColor(), RGBColor()}; //< Array que armazena a cor das 4 bordas do Background
  public:

    InterpoledBackground(const RGBColor color);                  //< Construtor Parametrizado com 1 cor
    InterpoledBackground(const std::array<RGBColor, 4> &colors); //< Construtor Parametrizado com 4 cores
